...
| `{{DESIGN_GUIDE_ROWS}}` | **Derived** (not collected): the `PATTERNS.md` index row for `experience-design.md`, emitted **only when that guide is installed** (the `design` preset). Empty otherwise — same must-not-index rule and newline handling as `{{QA_GUIDE_ROWS}}`. | the row if `guides/experience-design` is in the union, else empty |
...
| `{{QUALITY_GATES_SECTION}}` | **Derived** (not collected): the `CLAUDE.md` "Quality gates" section (contains the `{{GATE_COMMANDS}}` fence — substituted **before** the scalar/list pass so the nested placeholder still renders). Emitted **only when `guides/quality-gates` is in the union**; empty otherwise — same promise-only-what-is-installed rule and `\n\n` handling as `{{REVIEW_GATE_SECTION}}`. | the section iff `guides/quality-gates` is in the union, else empty |
| `{{ORCHESTRATION_STYLE_RULE}}` | **Derived** (not collected): the agent-registry "Multi-step work" orchestration-rules bullet. Names only orchestration commands that are in the union: both installed ⇒ the mode-dependent default rule; `commands/dispatch` only ⇒ the dispatch-only rule; `commands/pipeline-orchestrator` only ⇒ the pipeline-only rule; neither ⇒ the human-orchestrated fallback. Never empty. See `agentic-init/SKILL.md` Phase 4 step 4. | variant matching the installed command set |
| `{{BA_PO_GUIDE_ROWS}}` | **Derived** (not collected): the `PATTERNS.md` index rows for `ba-po-operating-model.md` and `mcp-onboarding.md`, each emitted **only when its guide is installed** (the `ba-po` preset — reaches `PATTERNS.md` in multi-role unions that include `governance/patterns`). Empty otherwise — same must-not-index rule and newline handling as `{{QA_GUIDE_ROWS}}`. | one row per installed guide of `guides/ba-po-operating-model` + `guides/mcp-onboarding`, else empty |
| `{{ENFORCEMENT_LAYER_ROWS}}` | **Derived** (not collected): the hook-backed rows of the `ai-policy.md` "Enforcement layers" table — pre-commit review stamp (iff `hooks/precommit-review-gate` **and** `githooks/pre-commit`), output-contract gate (iff `hooks/subagent-gate`), write-scope guard (iff `hooks/write-scope-guard`), instruction-quality gate (iff `hooks/instruction-gate`) — each emitted **only when its layer is in the union**, in that fixed order, each row ending in a newline. The two soft/settings rows stay in the template, so the table is never empty. The instruction-gate row nests `{{SCORECARD_PATH}}` — substitute **before** the scalar/list pass. See `agentic-init/SKILL.md` Phase 4 step 5. | one row per installed layer of the four |
| `{{FLEET_INVARIANTS}}` | **Derived** (not collected): the `AGENTS.md` "Fleet invariants" numbered list, renumbered contiguously so a union that skips an enforcement layer neither cites its hook nor leaves a numbering gap. Write-scope invariant always present (hook-citing variant iff `hooks/write-scope-guard`, else the stop-and-escalate variant); instruction-quality invariant **only when `hooks/instruction-gate` is in the union** (repair path names `instruction-auditor` iff `agents/instruction-auditor`, else a human re-grade); blind-review invariant **only when `hooks/precommit-review-gate` and `githooks/pre-commit` are both in the union**; the spawning, gate-agent, output-contract, and escalation invariants render in every install. Never empty. See `agentic-init/SKILL.md` Phase 4 step 4. | the invariant list matching the installed layer set |
...
| `{{HITL_MODE}}` | `strict` \| `gated-autonomous` \| `autonomous` | the preset union's strictest `default_hitl`, resolved strictest-wins (`strict > gated-autonomous > autonomous`) — Screen 3 pre-fill |
...
`guides/experience-design` (emotion-annotated journeys, decision-closing workshops,
negative ACs, context+spec handoff pair; pairs with `agents/experience-designer`).
...
`agents/experience-designer` (scoped doc-writer, `write_scope: docs/design/**`;
emotion-annotated journeys, proposed-only decisions, verbatim negative ACs; pairs
with `guides/experience-design`).
