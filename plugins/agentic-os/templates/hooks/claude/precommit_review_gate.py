...
read-only subagent (which is given ONLY the diff + a functional brief, never the
...
def gate() -> None:
    ...
    deny(
        ...
        "Before committing:\n"
        "1. Spawn the `blind-code-reviewer` subagent (Agent tool) on the STAGED "
        "diff. Pass it ONLY a one-paragraph functional brief of what the change "
        "should do — never your reasoning, rationale, or how you made decisions. "
        "It reads `git diff --cached` itself and reviews from scratch.\n"
