...
> **In a repo running `agentic-os`, this blueprint does not own the method.**
> The `security` preset installs a `threat-modeler` agent whose write scope,
> spawn gate and counted self-checks are enforced at runtime, against
> `.agentic/guides/standards/threat-modeling.md`. That standard is the
> authority: where it and this document differ, it wins and this document is
> what gets corrected. Use this blueprint for what the agent contract does not
> carry — standing up the *practice* across a team: role tiering, connector
> wiring, and the adoption signals that tell you it is working.
