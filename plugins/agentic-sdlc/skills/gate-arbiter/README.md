...
Shared gate-resolution helper. It does not run phases itself — `sdlc-engine`, `sdlc-brief`, and other orchestrating skills call it every time their flow hits a judgment gate, and it hands back a verdict plus an audit record.
...
> This skill is infrastructure. You don't call it by name; it's invoked internally at each gate inside `sdlc-engine` and `sdlc-brief` runs.
...
> Callers supply all four inputs per call. `gate-arbiter` doesn't define the full gate catalog or the schema of `context` beyond these four fields — that lives with the orchestrating skill.
