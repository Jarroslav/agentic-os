...
One-time-per-repo QA knowledge bootstrap. Scans the repo for test infrastructure, elicits your external test/QA integrations, and writes two persistent guides that `qa-scoping` reads on every feature run.

## Use It For

- Standing up the QA knowledge base before the first `sdlc-guided` or `sdlc-brief` run in a repo.
...
Both files are read automatically by `qa-scoping` on every feature run, feeding its checklist, test-review, and health-update modes.

> Non-goals: does not run tests or quality gates (a separate gates skill does that), and does not plan or review per-feature tests (that's `qa-scoping`).
