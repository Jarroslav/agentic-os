...
Deciding, at the pipeline's Phase 3 gate, whether a task is ready for planning as-is or needs a brainstorming pass beforehand. The skill carries no scoring logic of its own — it dispatches the `sizing-analyst` agent against the task and normalizes whatever that agent returns into a single score-plus-routing verdict that `sdlc-engine` acts on.

You will rarely need to call this directly. `sdlc-engine` dispatches it automatically as a fixed part of its Phase 3 flow, ahead of any planning or brainstorming work.
