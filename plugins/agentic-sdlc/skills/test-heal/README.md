...
application code, never re-runs the suite (that stays with `gate-runner`), and
...
- Fixing test-side failures (`test_issue`, `flaky`) that block `gate-runner`.
...
Invoked automatically by `sdlc-engine` Phase 10 (autonomous mode) when
`gate-runner` fails on test-side causes. Directly:
...
- The `gate_plan` from `gate-runner` (runner and commands — it never re-detects).
