...
> Gates run in order. The UI-test gate is conditional — it runs only when configured. This skill reports results only; it does not generate missing browser/UI coverage (that is acceptance-check's concern) and does not define retry/escalation logic.
...
- `sdlc-engine` at **Phase 8** (consumes the QA report for automated retry/escalation).
- `sdlc-brief` at **Stage 5**.
