...
- Proving a user-visible change renders and behaves after `gate-runner` passes.
...
Usually you don't. `sdlc-engine` auto-invokes this skill right after the `gate-runner` stage whenever a change touches a user-visible surface.
...
| Requirement | Detail |
| --- | --- |
| Predecessor run | `gate_plan` and `qa_report` from a `gate-runner` run |
