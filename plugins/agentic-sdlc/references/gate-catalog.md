...
Reference for engineers wiring `gate-arbiter`, `code-review-orchestrator`, `lead-proxy`, and
`sdlc-engine`. It is the single source of truth for gate IDs, what feeds each decision, and what
...
> never reaches `gate-arbiter`, a human, or a stand-in subagent while a deterministic answer is
> still available.
...
> `gate-arbiter` when confidence is high. A user override on this gate wins in either mode — it
...
| `classification.confirm` | 1 | `gate-arbiter` (HITL always asks; autonomous fast-paths on high confidence) | `requirements.md`, classification candidate, phase-set consequence | Low-confidence candidate; user override always wins |
...
> `code-review.final` and `code-review.check` don't route through `gate-arbiter` at all — the
...
on-cap behavior only — the counter's storage and resume mechanics are owned by `sdlc-engine`,
