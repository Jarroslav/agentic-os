...
expects, and that a fresh target repo scaffolds correctly after an explicit
`ba-po` role selection.
...
| `refinstall.py` | Deterministic `/agentic-init --presets ba-po` Phase 4 scaffold |
...
- Model-driven `/agentic-init` interview screens, MCP approval, and Phase 5 generation

After the automated run passes, open the target repo in Cursor with both plugins
installed and run `/agentic-init --presets ba-po` to complete the manual half.
